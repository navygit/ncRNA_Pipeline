...
%Config = (
  ...
  PIPELINE_TARGET_DIR => '/nfs/nobackup/ensemblgenomes/production/protein_pipelines/data/cmerolae/',

  #regex for spiting up slice input_ids
  SLICE_INPUT_ID_REGEX => '(\S+)\.(\d+)-(\d+):?([^:]*)',	   
    
  ANALYSIS_WORK_DIR => '/nfs/nobackup/ensemblgenomes/production/protein_pipelines/data/cmerolae/',
