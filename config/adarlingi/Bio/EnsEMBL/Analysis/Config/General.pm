...
%Config = (
    ...
    PIPELINE_TARGET_DIR => '/nfs/nobackup2/ensemblgenomes/'.$ENV{USER}.'/dna_pipelines/data/adarlingi',
    
    #regex for spiting up slice input_ids
    SLICE_INPUT_ID_REGEX => '(\S+)\.(\d+)-(\d+):?([^:]*)',	   
    
    ANALYSIS_WORK_DIR => '/nfs/nobackup2/ensemblgenomes/'.$ENV{USER}.'/dna_pipelines/data/adarlingi',
